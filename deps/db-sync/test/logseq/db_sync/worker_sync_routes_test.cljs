...
(deftest match-route-sync-test
  (testing "sync routes"
    ...
    (let [match (sync-routes/match-route "GET" "/snapshot/download-v2")]
      (is (= :sync/snapshot-download-v2 (:handler match))))
    (let [match (sync-routes/match-route "DELETE" "/snapshot/download-v2")]
      (is (= :sync/snapshot-download-v2-cancel (:handler match))))
    (let [match (sync-routes/match-route "GET" "/snapshot/stream-v2")]
      (is (= :sync/snapshot-stream-v2 (:handler match))))
    ...
    (let [match (sync-routes/match-route "POST" "/snapshot/upload-v2")]
      (is (= :sync/snapshot-upload-v2 (:handler match))))))
...
(deftest match-route-sync-method-mismatch-test
  (testing "sync method mismatch returns nil"
    ...
    (is (nil? (sync-routes/match-route "POST" "/snapshot/download-v2")))
