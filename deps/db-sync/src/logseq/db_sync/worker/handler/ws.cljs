            ...
            (let [authorized? (<connection-authorized? self ws)
              ...
              (if (p/promise? authorized?)
                (p/let [allowed? authorized?]
                  (continue! allowed?))
                (continue! authorized?))))]
    (let [graph-id (or (presence/get-graph-id self ws)
                       (aget self "graph-id"))
          integrity (sync-handler/<ensure-live-integrity! self graph-id)]
      (if (p/promise? integrity)
        (-> (p/let [_ integrity]
              (continue-authorized!))
            (p/catch
             (fn [_]
               (ws/send! ws {:type "error"
                             :message "snapshot integrity unavailable"}))))
        (continue-authorized!)))))
