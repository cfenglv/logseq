...
(defonce *auto-repair-inflight (atom nil))

(declare quarantine-after-snapshot-failure!)
...
(defn- load-auto-repair-receipt
  [repo]
  (try
    (client-op/get-auto-repair-attempt repo)
    (catch :default error
      ;; This receipt only governs destructive automatic replacement. A
      ;; missing/legacy/unavailable sidecar must not prevent an ordinary
      ;; fail-safe RTC connection from starting.
      (log/warn :db-sync/automatic-repair-receipt-load-failed
                {:repo repo
...
(defn- ensure-client-state!
  ...
  {:repo repo
   ...
   ;; Read the durable receipt once per client generation. Normal successful
   ;; tx/hello/pull handling must not issue a DELETE against the sidecar.
   :auto-repair-receipt (atom (load-auto-repair-receipt repo))
...
(defn- auto-repair-work-state
  [repo]
  (let [pending-local (client-op/get-durable-pending-local-tx-count repo)]
    {:pending-asset (client-op/get-unpushed-asset-ops-count repo)
     :pending-local pending-local
     :pending-overlay (when (and (integer? pending-local)
                                 (pos? pending-local))
                        (sync-apply/pending-txs repo))
     :missing-assets (sync-assets/get-missing-asset-upload-files repo)}))

(defn- safe-to-replace-local-snapshot?
  [{:keys [pending-asset pending-local pending-overlay missing-assets]}]
  (and (integer? pending-local)
       (or (zero? pending-local)
           (and (= pending-local (count pending-overlay))
                (sync-apply/repairable-automatic-repair-overlay?
                 pending-overlay)))
       (integer? pending-asset)
       (zero? pending-asset)
       (empty? missing-assets)))

(defn- assert-auto-repair-owner!
  [repo graph-id client]
  (let [current @worker-state/*db-sync-client
        current-target [(:repo current) (:graph-id current)]
        expected-target [repo graph-id]]
    (when-not (and (identical? client current)
                   (= expected-target current-target))
      (throw
       (ex-info "automatic snapshot repair no longer owns the current graph"
                {:type :db-sync/stale-auto-repair-generation
                 :repo repo
                 :graph-id graph-id})))
    true))

(defn- assert-auto-repair-activation-safe!
  [repo graph-id client captured-overlay]
  (assert-auto-repair-owner! repo graph-id client)
  (let [work-state (auto-repair-work-state repo)
        current-overlay (or (:pending-overlay work-state) [])]
    (when-not (and (safe-to-replace-local-snapshot? work-state)
                   (= captured-overlay current-overlay))
      (throw
       (ex-info "local work appeared while automatic snapshot repair was staging"
                {:type :db-sync/auto-repair-local-work-detected
                 :repo repo
                 :pending-asset (:pending-asset work-state)
                 :pending-local (:pending-local work-state)
                 :missing-assets-count (count (:missing-assets work-state))}))))
  true)

(defn- maybe-start-auto-repair!
  [repo client error]
  (try
    (let [graph-id (:graph-id client)
          work-state (auto-repair-work-state repo)
          captured-overlay (or (:pending-overlay work-state) [])
          target [repo graph-id]]
      (when (and (seq graph-id)
                 (nil? @*auto-repair-inflight)
                 (safe-to-replace-local-snapshot? work-state)
                 (client-op/claim-auto-repair-attempt!
                  repo graph-id (error-type error)))
        (let [receipt (client-op/get-auto-repair-attempt repo)
              _ (when-not (map? receipt)
                  (throw
                   (ex-info "automatic repair receipt was not durable"
                            {:type :db-sync/auto-repair-receipt-unavailable
                             :repo repo
                             :graph-id graph-id})))
              replacement-receipt
              (cond-> receipt
                (seq captured-overlay)
                (assoc :overlay-tx-ids (mapv (comp str :tx-id)
                                             captured-overlay)))
              repair-promise
              (-> (p/resolved nil)
                  (p/then
                   (fn []
                     (let [graph-e2ee? (sync-crypt/graph-e2ee? repo)]
                       (when (identical? client @worker-state/*db-sync-client)
                         (set-sync-state! client :repairing false))
                       (sync-download/download-graph-by-id!
                        repo graph-id graph-e2ee?
                        {:activation-preflight-f
                         (fn []
                           (assert-auto-repair-activation-safe!
                            repo graph-id client captured-overlay))
                         :post-backup-pre-reset-f
                         (fn []
                           ;; The activation gate is held here. Recheck after
                           ;; the durable backup finished so an edit that raced
                           ;; the copy can never be overwritten by that backup.
                           (assert-auto-repair-activation-safe!
                            repo graph-id client captured-overlay))
                         :post-activation-precommit-f
                         (fn []
                           (assert-auto-repair-owner! repo graph-id client)
                           ;; The replacement client-op DB must retain the
                           ;; original one-shot circuit receipt before the
                           ;; local backup is committed.
                           (client-op/preserve-auto-repair-attempt!
                            repo replacement-receipt)
                           (when (seq captured-overlay)
                             (sync-apply/restore-automatic-repair-overlay!
                              repo captured-overlay)))
                         :failure-handler quarantine-after-snapshot-failure!}))))
                  (p/then (fn [_]
                            (assert-auto-repair-owner! repo graph-id client)
                            ;; Opening a new socket is not the health proof. The
                            ;; new generation remains non-ready until hello/pull
                            ;; and both checksums converge.
                            (when (= target (:target @*auto-repair-inflight))
                              (reset! *auto-repair-inflight nil))
                            (start! repo)))
                  (p/catch
                   (fn [repair-error]
                     (log/error :db-sync/automatic-repair-failed
                                {:repo repo
                                 :graph-id graph-id
                                 :error-type (error-type repair-error)})
                     (when-let [current @worker-state/*db-sync-client]
                       (when (and (= repo (:repo current))
                                  (= graph-id (:graph-id current)))
                         (sync-util/set-last-sync-error! current repair-error)
                         (set-sync-state! current :repair-required false)))
                     nil)))]
          (reset! *auto-repair-inflight
                  {:target target :promise repair-promise})
          (p/finally repair-promise
                     (fn []
                       (when (= target (:target @*auto-repair-inflight))
                         (reset! *auto-repair-inflight nil)))))))
    (catch :default claim-error
      ;; Receipt/storage problems must fail closed without escaping the socket
      ;; callback or creating a reconnect loop.
      (log/error :db-sync/automatic-repair-claim-failed
                 {:repo repo
                  :graph-id (:graph-id client)
                  :error-type (error-type claim-error)})
      nil)))
...
(defn- enter-repair-required!
  ...
  ;; This is a one-shot persisted recovery attempt. Failure keeps the local DB
  ;; editable and parked in repair-required; visibility/network events cannot
  ;; re-arm it.
  (maybe-start-auto-repair! repo client error))

(defn- clear-converged-auto-repair-receipt!
  [repo client]
  (when-let [receipt-atom (:auto-repair-receipt client)]
    (when-let [receipt @receipt-atom]
      (when (and (= :started (:status receipt))
                 (= (str (:graph-id client)) (:graph-id receipt))
                 ;; A zero-overlay repair is complete after canonical catch-up.
                 ;; An overlay repair stays armed until every preserved local
                 ;; intent has an authoritative ACK.
                 (or (not (contains? receipt :overlay-tx-ids))
                     (zero?
                      (client-op/get-durable-pending-local-tx-count repo)))
                 (client-op/clear-auto-repair-attempt-if-matches!
                  repo receipt))
        (reset! receipt-atom nil)
        true))))
...
(defn- stop-client!
  ...
  (when-let [reconnect (:reconnect client)]
    (clear-reconnect-timer! reconnect))
  ;; State must become non-ready even when the socket has already disappeared.
  ;; Previously this lived inside `when-let ws`, leaving an open/ready payload
  ;; behind when a snapshot failure raced with WebSocket teardown.
  (update-online-users! client [])
  (set-sync-state! client :closed false)
  (when-let [ws (:ws client)]
    (detach-ws-handlers! ws)
...
(defn quarantine-after-snapshot-failure!
  "Retire a client that survived a failed destructive snapshot activation.
  Durable pending client ops are intentionally preserved; only the obsolete
  socket generation, timers, and promise queues are replaced."
  [repo graph-id error]
  (when-let [client @worker-state/*db-sync-client]
    (when (and (= repo (:repo client))
               (= (str graph-id) (str (:graph-id client))))
      (let [ws (:ws client)]
        (clear-stale-ws-loop-timer! client)
        (sync-apply/clear-upload-response-timeout! client)
        (clear-inflight! client)
        ...
        (when ws
          (detach-ws-handlers! ws)
          (try (.close ws) (catch :default _ nil)))
        ;; Old receive tasks compare their captured connection generation with
        ;; the current one and become no-ops. Old send tasks retain only the
        ;; closed socket. New work cannot join either retired promise chain.
        (when (identical? client @worker-state/*db-sync-client)
...
;; Keep direct worker-side snapshot callers fail-safe without making the
;; download namespace depend back on this client namespace.
(sync-download/register-snapshot-failure-handler!
 quarantine-after-snapshot-failure!)
                         ...
                         (fn []
                           (when-let [current @worker-state/*db-sync-client]
                             (when (and (= repo (:repo current))
                               ...
                               (clear-converged-auto-repair-receipt!
                                repo current)
...
(defn start!
  ...
  (let [base (ws-base-url)
        ...
        current-before-start @worker-state/*db-sync-client
        auto-repair @*auto-repair-inflight]
    ;; Visibility resume never reaches `start!` for repair-required. Reaching
    ;; this branch from the UI is the explicit action that re-arms one bounded
    ;; repair attempt after the operator has inspected the graph.
    (when (and current-before-start
               (= repo (:repo current-before-start))
               (= :repair-required
                  (some-> current-before-start :ws-state deref))
               (nil? @*auto-repair-inflight))
      (try
        (client-op/clear-auto-repair-attempt! repo)
        (catch :default clear-error
          (log/warn :db-sync/automatic-repair-rearm-failed
                    {:repo repo
                     :error-type (error-type clear-error)}))))
    (if (= [repo initial-graph-id] (:target auto-repair))
      ;; UI retries, graph visibility events, and duplicate starts join the
      ;; same one-shot repair instead of interrupting it with a new socket.
      (:promise auto-repair)
