       ...
       (let [graph-e2ee? (normalize-graph-e2ee? graph-e2ee?)
         ...
         (-> (if (and graph-uuid base)
               (p/let [_ (js/Promise. user-handler/task--ensure-id&access-token)
                       ...
                       ;; Snapshot activation replaces the local graph and its
                       ;; client-op cursor. Quiesce the existing socket,
                       ;; reconnect loop, and volatile queues before the worker
                       ;; starts importing the replacement basis.
                       _ (<rtc-stop!)
