...
(deftest cut-shortcut-clears-selection-only-after-the-cut-settles-test
  (async done
    (let [cut-finished (p/deferred)
          calls (atom [])]
      (-> (p/with-redefs [state/get-state (constantly nil)
            ...
            (let [request (#'editor/cut-blocks-and-clear-selections! true)]
              (p/let [_ (p/delay 0)
                      _ (is (= [[:cut true]] @calls)
                            "Selection must remain available while the async cut resolves it to an embed placeholder.")
                      _ (p/resolve! cut-finished true)
                      _ request]
                (is (= [[:cut true] :clear-selection] @calls)))))
          (p/finally done)))))
