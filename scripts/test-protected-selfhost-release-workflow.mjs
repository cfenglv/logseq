...
addCase("desktop release is manual-only and cannot run on branch push", () => {
  const preamble = workflow.slice(0, workflow.indexOf("env:"));
  assert.doesNotMatch(preamble, /^  push:/m);
  assert.match(preamble, /^  workflow_dispatch:/m);
});

addCase("release source gate binds the manually selected ref to the exact SHA", () => {
  const sourceGate = workflowJob("release-source-gate");
  assert.match(sourceGate, /source-ref:[\s\S]*steps\.source\.outputs\.ref/);
  assert.match(sourceGate, /source-sha:[\s\S]*steps\.source\.outputs\.sha/);
  assert.match(sourceGate, /REQUESTED_SOURCE_REF:[\s\S]*github\.event\.inputs\.git-ref/);
  assert.doesNotMatch(sourceGate, /listWorkflowRuns|event:\s*'push'/);
});
...
addCase("protected signer is exact-ref workflow_dispatch stable/beta only", () => {
  ...
  for (const needle of [
    ...
    "github.ref_name == needs.release-source-gate.outputs.source-ref",
    "github.sha == needs.release-source-gate.outputs.source-sha",
  ...
  assert.match(
    signer,
    /needs:\s*\[\s*release-assets-preflight,\s*release-source-gate\s*\]/,
  );
...
addCase("secretless verifier rechecks complete assets and signatures", () => {
  ...
  assert.match(
    verifier,
    /needs:\s*\[\s*selfhost-release-signing,\s*release-source-gate\s*\]/,
  );
  ...
  assert.match(
    verifier,
    /--source-revision[\s\S]{0,160}release-source-gate\.outputs\.source-sha/,
  );
  assert.match(
    verifier,
    /source-ref:\s*\$\{\{ needs\.release-source-gate\.outputs\.source-ref \}\}/,
  );
  assert.match(
    verifier,
    /source-sha:\s*\$\{\{ needs\.release-source-gate\.outputs\.source-sha \}\}/,
  );
  assert.match(
    verifier,
    /ref:\s*\$\{\{ needs\.release-source-gate\.outputs\.source-sha \}\}/,
  );
  assert.match(
    verifier,
...
addCase("publisher has separate protected write boundary after verifier", () => {
  ...
  assert.match(publisher, /files:\s*release-assets\/\*/);
  assert.doesNotMatch(publisher, /release-assets\/\*\.apk/);
  assert.match(
    workflowJob("selfhost-release-verifier"),
    /verify-finalized-selfhost-release\.mjs[\s\S]*--source-revision/,
  );
  assert.doesNotMatch(
    publisher,
