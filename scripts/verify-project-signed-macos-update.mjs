...
export const verifyProjectSignedMacosUpdate = ({
  arch,
  archive,
  metadata,
  version,
  ...
  const field = (name) => {
    const value = yaml
      ...
      ?.trim();
    if (!value) {
      throw new Error(`metadata projectUpdateSignature is missing ${name}`);
    }
    return value;
  };
...
if (isEntrypoint) {
  ...
  console.log(
    `[project-update-verify] OK version=${result.version} arch=${result.arch} keyId=${result.keyId}`,
  );
}
