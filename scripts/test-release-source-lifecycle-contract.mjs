...
test("provided release identity values are raw exact lowercase SHAs", () => {
  withRepository((root) => {
    ...
    establishReleaseSourceIdentity({ environment: revisionOnly, repoRoot: root });
...
test("tracked build-time source changes fail the closing identity check", () => {
  withRepository((root) => {
    const environment = {};
    const identity = establishReleaseSourceIdentity({ environment, repoRoot: root });
...
test("untracked build-time source changes fail the closing identity check", () => {
  withRepository((root) => {
    const environment = {};
    const identity = establishReleaseSourceIdentity({ environment, repoRoot: root });
    fs.writeFileSync(path.join(root, "late-source.mjs"), "export default true;\n");
    assert.throws(
      () =>
        assertReleaseSourceIdentityUnchanged(identity, {
          environment,
