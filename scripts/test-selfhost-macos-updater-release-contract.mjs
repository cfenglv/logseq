...
function loadUpdaterTestRuntime(runtimePackageRoot) {
  const packageRequire = createRequire(
    path.join(runtimePackageRoot, 'package.json')
  )
  const updaterRequire = createRequire(
    packageRequire.resolve('electron-updater/package.json')
  )

  return {
    GitHubProvider: packageRequire(
      'electron-updater/out/providers/GitHubProvider'
    ).GitHubProvider,
    HttpError: updaterRequire('builder-util-runtime').HttpError,
    semver: packageRequire('semver'),
  }
}

const { GitHubProvider, HttpError, semver } =
  loadUpdaterTestRuntime(packageRoot)
...
test('updater test runtime does not depend on pnpm hoisting', () => {
  const strictRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), 'logseq-updater-strict-runtime-')
  )
  try {
    const strictNodeModules = path.join(strictRoot, 'node_modules')
    fs.mkdirSync(strictNodeModules)
    fs.writeFileSync(
      path.join(strictRoot, 'package.json'),
      '{"name":"logseq-updater-strict-runtime","private":true}\n'
    )

    const installedRuntimeRequire = createRequire(
      path.join(packageRoot, 'package.json')
    )
    for (const dependency of ['electron-updater', 'semver']) {
      const packageDirectory = path.dirname(
        installedRuntimeRequire.resolve(`${dependency}/package.json`)
      )
      fs.symlinkSync(
        packageDirectory,
        path.join(strictNodeModules, dependency),
        process.platform === 'win32' ? 'junction' : 'dir'
      )
    }

    const strictRequire = createRequire(path.join(strictRoot, 'package.json'))
    assert.throws(
      () => strictRequire.resolve('builder-util-runtime'),
      (error) => error?.code === 'MODULE_NOT_FOUND',
      'fixture must reproduce strict pnpm isolation without a hoisted transitive dependency'
    )

    const strictRuntime = loadUpdaterTestRuntime(strictRoot)
    assert.equal(strictRuntime.GitHubProvider, GitHubProvider)
    assert.equal(strictRuntime.HttpError, HttpError)
    assert.equal(strictRuntime.semver, semver)
  } finally {
