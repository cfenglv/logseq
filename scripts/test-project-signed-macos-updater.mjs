...
const electronFixture = () => {
  const packageRoot = process.env.LOGSEQ_UPDATER_TEST_PACKAGE_ROOT?.trim();
  const candidates = [
...
let sevenZipTimestampOptionsSupported;

const sevenZipAdd = ({ archive, cwd, entries }) => {
  ...
  if (sevenZipTimestampOptionsSupported !== false) {
    const archiveBefore = fs.existsSync(archive)
    ...
    const deterministicResult = command(executable, deterministicArgs, {
      allowFailure: true,
      cwd,
    });
    if (deterministicResult.status === 0) {
      sevenZipTimestampOptionsSupported = true;
      return;
    }
    sevenZipTimestampOptionsSupported = false;
  ...
  command(executable, commonArgs, { cwd });
};
...
const addMacosxSidecarEntry = ({ archive, root }) => {
  ...
  sevenZipAdd({ archive, cwd: sidecarRoot, entries: ["__MACOSX"] });
