...
addCase(
  ...
  () => {
    ...
    assert.ok(needs.includes("release-source-gate"));
...
addCase(
  "D push, PR, nightly, nonselfhost, and unpinned refs cannot sign or publish",
  () => {
    const workflow = read(workflowRelativePath);
    const jobs = workflowJobs(workflow);
    const workflowPreamble = workflow.slice(0, workflow.indexOf("jobs:"));
    assert.doesNotMatch(workflowPreamble, /^\s*pull_request:/m);
    const sourceGate = jobs.get("release-source-gate") ?? "";
    assert.match(
      sourceGate,
      /^    outputs:\s*\n(?:^      [^\n]+\n)*^      (?:sha|source-sha|source_sha):\s*\$\{\{\s*steps\.source\.outputs\.sha\s*\}\}/m,
      "source gate does not expose its exact reviewed source SHA",
    );
    const signing = singleJobWithEnvironment(jobs, signingEnvironmentName);
    ...
    assert.ok(jobNeeds(signing.source).includes("release-source-gate"));
    assert.match(
      signing.source,
      /needs\.release-source-gate\.outputs\.(?:sha|source-sha|source_sha)/,
      "signing job is not pinned to the selected source SHA",
    );
...
addCase(
  ...
  () => {
    ...
    assert.match(
      signing.source,
      /needs\.release-source-gate\.outputs\.(?:sha|source-sha|source_sha)/,
      "signing job does not consume the selected source SHA",
    );
    assert.match(
      signing.source,
      /(?:--(?:expected-)?source-(?:sha|revision)|EXPECTED_SOURCE_(?:SHA|REVISION)|SOURCE_SHA)[\s\S]{0,160}(?:needs\.release-source-gate\.outputs\.(?:sha|source-sha|source_sha))|(?:needs\.release-source-gate\.outputs\.(?:sha|source-sha|source_sha))[\s\S]{0,160}(?:--(?:expected-)?source-(?:sha|revision)|EXPECTED_SOURCE_(?:SHA|REVISION)|SOURCE_SHA)/,
      "GitHub finalizer is not passed the exact selected source SHA",
    );
    ...
    assert.ok(
      jobNeeds(finalVerifier).includes("release-source-gate"),
      "final verifier does not depend on the exact selected SHA",
    );
    assert.match(
      finalVerifier,
      /(?:--(?:expected-)?source-(?:sha|revision)|EXPECTED_SOURCE_(?:SHA|REVISION)|SOURCE_SHA)[\s\S]{0,160}(?:needs\.release-source-gate\.outputs\.(?:sha|source-sha|source_sha))|(?:needs\.release-source-gate\.outputs\.(?:sha|source-sha|source_sha))[\s\S]{0,160}(?:--(?:expected-)?source-(?:sha|revision)|EXPECTED_SOURCE_(?:SHA|REVISION)|SOURCE_SHA)/,
      "final verifier is not passed the exact selected SHA",
    );
    ...
    assert.match(
      publishing.source,
      /files:\s*release-assets\/\*/,
      "published release does not include the complete verified asset directory",
    );
  },
);
...
addCase(
  ...
  () => {
    ...
    assert.match(
      publishing.source,
      /fail_on_unmatched_files:\s*true[\s\S]*files:\s*release-assets\/\*/,
      "selfhost GitHub Release does not publish the complete verified directory",
    );
    assert.doesNotMatch(
      publishing.source,
      /release-assets\/\*\.apk/,
      "optional Android assets must not block a desktop-only release",
    );
  },
);
