...
assert.ok(
  new Set(["2.0.1-selfhost.7", "2.0.1-selfhost.8"]).has(targetVersion),
  "candidate version must be the formal .7 or synthetic .8",
);

const sourcePath = path.join(repoRoot, "src/main/frontend/version.cljs");
const source = fs.readFileSync(sourcePath, "utf8");
const pattern = /\(defonce version "2\.0\.1-selfhost\.[78]"\)/;
